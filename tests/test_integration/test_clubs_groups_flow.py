...
def test_club_flow(client, monkeypatch):
    ...
    # Bypass creation limits for integration tests
    # Must patch at the location where they're imported, not at the source
    with patch('app.routers.clubs.check_club_creation_limit', return_value=(True, 0, 10)), \
         patch('app.routers.groups.check_group_creation_limit', return_value=(True, 0, 10)):
        try:
            # 1. Create Club
            club_data = {
...
def test_standalone_group_flow(client, monkeypatch):
    ...
    # Bypass creation limits for integration tests
    with patch('app.routers.groups.check_group_creation_limit', return_value=(True, 0, 10)):
        try:
