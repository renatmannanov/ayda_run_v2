...
// Sample data - демонстрация статусов после тренировки
const sampleActivities = [
  {
    ...
    participants: 5,
    maxParticipants: 15,
    status: 'awaiting', // тренировка прошла, ждём подтверждения
    isPrivate: false,
    hasGpx: false
  },
  {
    ...
    participants: 12,
    maxParticipants: 20,
    status: 'attended', // участвовал
    isPrivate: false,
    hasGpx: false
  },
  {
    id: 3,
    title: "Длинная воскресная",
...
const StatusButton = ({ status, isPrivate, onClick }) => {
  // Attended - green outlined circle with green checkmark
  if (status === 'attended') {
    return (
...
const ActivityCard = ({ activity, onClick }) => {
  const isPast = activity.status === 'attended' || activity.status === 'missed';
  ...
  return (
    <div 
      onClick={onClick}
      className={`bg-white border border-gray-200 rounded-xl p-4 mb-3 cursor-pointer hover:border-gray-300 transition-colors ${isPast ? 'opacity-50' : ''}`}
    >
      {/* Title + Sport Icon */}
